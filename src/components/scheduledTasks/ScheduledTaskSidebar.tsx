  ...
  return (
    <aside className="flex w-[320px] flex-shrink-0 flex-col border-r border-claude-border bg-claude-panel">
      <div className="flex items-center justify-between px-4 py-3">
        <div>
          <p className="text-sm font-semibold text-claude-text">{t('scheduled.sidebar.title')}</p>
          <p className="text-xs text-claude-muted">{t('scheduled.sidebar.registered', { count: tasks.length })}</p>
        </div>
        <button
          onClick={onCreate}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3">
        {tasks.length === 0 ? (
          <div className="flex h-full items-center justify-center rounded-[12px] border border-dashed border-claude-border bg-claude-bg px-6 py-8 text-center">
            <div>
              <p className="text-sm font-semibold text-claude-text">{t('scheduled.sidebar.emptyTitle')}</p>
              <p className="mt-1 text-xs leading-relaxed text-claude-muted">{t('scheduled.sidebar.emptyDescription')}</p>
              <button
                type="button"
                onClick={onCreate}
                ...
                >
                  ...
                  <div className="mt-2 flex items-center justify-between gap-3 text-xs">
                    <span className="truncate text-claude-muted">
                      {t('scheduled.sidebar.nextRun')} <span className="text-claude-text">{formatDateTime(task.nextRunAt, language)}</span>
                    </span>
                    <button
                      type="button"
                      onClick={(event) => {
