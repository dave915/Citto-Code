...
export function TimelineEntryRow({
  ...
  const showCodePreview = primaryTool.toolName === 'Read' && resultText.trim().length > 0
  ...
  const handleOpenSubagentSession = async (event: ReactMouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (!session) {
        setSubagentSessionError(t('subagent.transcriptUnavailable'))
        return
      }
      ...
      setSubagentSessionError(t('subagent.openSessionFailed'))
    } finally {
    ...
    <div className="space-y-1">
      <div className="flex items-start gap-1.5 text-[14px] leading-5 text-claude-muted">
        <button
          ...
          className="mt-[1px] flex h-4 w-4 items-center justify-center text-claude-muted outline-none focus:outline-none focus-visible:ring-1 focus-visible:ring-claude-orange/35"
        >
          ...
          <div className="flex min-w-0 items-center gap-1.5">
            ...
            {entry.badge && (
              <button
                type="button"
                onClick={() => setExpanded((value) => !value)}
                className="inline-flex max-w-[min(38rem,62vw)] items-center gap-1 truncate rounded-md bg-black/20 px-1.5 py-0.5 font-mono text-[10px] leading-4 text-claude-text/90 outline-none transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-claude-orange/35"
              >
        ...
        <div className="ml-[10px] border-l border-claude-border/70 pl-3">
          {entry.kind === 'todo' ? (
            ...
            <div className="space-y-1 rounded-lg border border-claude-border/70 bg-claude-bg px-3 py-2">
              <div className="text-[11px] font-medium text-claude-text/80">{t('toolTimeline.subagentSession')}</div>
            ...
            <div className="text-[11px] text-claude-muted">{t('toolTimeline.loadingFileContents')}</div>
          ) : showCodePreview ? (
            <div className="space-y-1">
              {(() => {
